/*
 * mini-agent-c v9: v8 + parallel tools, ask_user, git, diff_files, notify,
 *                      streaming bash, extended thinking, auto-mkdir.
 *
 * New in v9:
 *  - Parallel tool execution                  — fork() multiple tools, collect results
 *  - ask_user(question, default)              — mid-task user prompts via /dev/tty
 *  - diff_files(path_a, path_b)               — diff -u between two files
 *  - git(subcommand, args)                    — safe git operations
 *  - notify(title, message)                   — macOS notifications via osascript
 *  - --stream-bash                            — real-time bash output to stderr
 *  - --think / --think-budget                 — extended thinking mode
 *  - Auto-mkdir in write_file                 — creates parent directories
 *  - --no-parallel                            — disable parallel tool execution
 *
 * Inherited from v8:
 *  - grep_files, glob_files, http_get, todo, SIGINT, cost tracking, REPL
 *
 * Inherited from v7:
...
static cJSON *build_system_array(const char *memory) {
    ...
    cJSON_AddStringToObject(base, "text",
        "You are mini-agent-c v8, a minimal autonomous agent in pure C. "
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (strcmp(a, "--version") == 0) { printf("mini-agent-c v9.0\n"); return 0; }
