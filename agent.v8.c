/*
 * mini-agent-c v8: v7 + grep_files, glob_files, http_get, todo, SIGINT,
 *                      cost tracking, interactive REPL mode.
 *
 * New in v8:
 *  - grep_files(pattern, path, include_glob)  — regex search across files
 *  - glob_files(pattern, path)                — find files by glob
 *  - http_get(url)                            — fetch URLs (--allow-http)
 *  - todo(op, item)                           — task list in .agent/todo.md
 *  - SIGINT handler                           — Ctrl-C exits cleanly
 *  - Cost tracking                            — shows $ estimate per session
 *  - --interactive / -i                       — REPL for multi-task sessions
 *
 * Inherited from v7:
    ...
    for (turn = 0; turn < g_max_turns; turn++) {
        ...
        cJSON_ArrayForEach(block, content) {
            ...
            if (strcmp(type, "text") == 0) {
                ...
                cJSON *inp       = cJSON_GetObjectItem(block, "input");
...
static void usage(const char *prog) {
    fprintf(stderr,
        ...
        "  --help              show this help\n"
        "\n"
        "New tools in v8: grep_files, glob_files, http_get, todo\n",
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (strcmp(a, "--version") == 0) { printf("mini-agent-c v8.0\n"); return 0; }
