/*
 * mini-agent-c v7: v6 + precise edit_file tool (Claude Code-style edit).
 *
 * Features:
...
static char *tool_edit_file(const char *path, const char *old_str, const char *new_str, int replace_all) {
    ...
    if (!old_str || !*old_str) {
        char *r = malloc(128);
        snprintf(r, 128, "Error: old_string is empty");
        return r;
    }
    if (!new_str) new_str = "";
    ...
    if (count > 1 && !replace_all) {
        free(buf);
        char *r = malloc(256);
        snprintf(r, 256, "Error: old_string matches %d places in %s; add surrounding context to make it unique, or set replace_all=true", count, path);
        return r;
    }
...
static cJSON *build_tools_array(void) {
    ...
    {
        /* edit_file: path, old_string, new_string required; replace_all optional */
        const char *ep[]  = {"path", "old_string", "new_string"};
        const char *epd[] = {
            "Relative path inside workspace",
            "Exact substring to replace (must be unique unless replace_all=true)",
            "Replacement text"
        };
        cJSON *edit = make_simple_tool("edit_file",
            "Precise in-place edit: replace old_string with new_string in an existing file. "
...
static cJSON *build_system_array(const char *memory) {
    ...
    cJSON_AddStringToObject(base, "text",
        "You are mini-agent-c v7, a minimal autonomous agent implemented in ~1700 lines of C. "
        "You have tools: read_file, write_file, edit_file, bash, list_dir, save_memory, recall_memory, spawn_agent, "
        "plus any dynamic tools loaded from .agent/tools/*.sh. "
        "Prefer edit_file over write_file when changing an existing file: it is safer and preserves the rest of the file. "
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (strcmp(a, "--version") == 0) { printf("mini-agent-c v7.0\n"); return 0; }
